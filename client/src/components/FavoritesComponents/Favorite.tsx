  ...
  return (
    <Box sx={{width: {xs: "100%", sm: "100%", md: "80%"}, margin: "0 auto", my: 5 }}>
        <Box sx={{display: "flex", justifyContent: "space-between", p: 1}}>
            ...
            <Button sx={{display: user ? "none": "flex" ,borderRadius: "20px",p :1, width: "100px", border: "2px solid black", color:"black"}}>Sign in</Button>
        </Box>
        <Typography sx={{p:1, display: user ? "none": "flex"}}>Favorites only stick around for 7 days (or until you clear your cache). To keep them longer and view them from any device, sign in or create an account.</Typography>
        <Box sx={{display: user ? "none": "flex", justifyContent: "space-between", alignItems: "center",p: 1}}>
        ...
        <Box sx={{display: "grid", gridTemplateColumns: {xs: "1fr 1fr", md: "1fr 1fr 1fr 1fr 1fr"}, justifyItems: "center", alignItems: "center", justifyContent: "space-around", alignContent: "center", gridAutoFlow: "row", my: 5, p:2}}>
        {
          favorites?.products?.map((item: Product) => {
            return (
              <FavoriteItem key={item._id} product={item} />
            )
          })
        }
        </Box>
        ...
        <Box sx={{display: {xs: favorites.products.length > 0 ? "none" : "flex", sm: favorites.products.length > 0 ? "none": "flex"}, flexDirection: "column", justifyContent: "center", alignItems: "center", jusstifySelf: "center", textAlign: "center", p: {xs: 5, sm: 10}}}>
